    ...
    @if(auth()->check())
        @if(auth()->user()->banned_until && now()->isBefore(auth()->user()->banned_until))
            ...
            <script>
                ...
                console.log("Banned Until Time (ms):", bannedUntil); 
                        ...
                        <div class="bg-gray-700 p-4 rounded-lg shadow-md">
                            <div class="flex items-center mb-2">
                                <div class="flex flex-col items-center group relative mr-4">
                                    <div class="small-card" style="background-image: url('{{ asset('storage/images/' . $reservation->movie->poster_filename) }}');"></div>
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-300">
                                        ...
                                        @if($reservation->status === 'approved')
                                            <span class="text-green-500 font-bold">{{ ucfirst($reservation->status) }}</span>
                                        @elseif($reservation->status === 'denied')
                                            <span class="text-red-500 font-bold">{{ ucfirst($reservation->status) }}</span>
                                        @else
                                            <span class="text-yellow-500 font-bold">{{ ucfirst($reservation->status) }}</span>
                                        @endif
                                    </p>
                                    
                                    @if($reservation->status === 'approved')
                                        <button class="mt-2 bg-gray-500 text-white font-bold py-2 px-4 rounded cursor-not-allowed" 
                                                title="This reservation has been approved and may not be eligible for cancellation." 
                                                disabled>
                                            Cancel Purchase
                                        </button>
                                    @else
                                        <button onclick="confirmCancel({{ $reservation->id }})" class="mt-2 bg-red-500 text-white font-bold py-2 px-4 rounded">
                                            Cancel Purchase
                                        </button>
                                    @endif
                                </div>    
